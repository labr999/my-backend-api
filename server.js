...
function fetchTextOnce(url){
  return new Promise((resolve,reject)=>{
    const args=['-L','--max-time','10','--connect-timeout','5','--retry','0','--compressed','-A','Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/151 Safari/537.36','-H','Accept-Language: zh-TW,zh;q=0.9,zh-CN;q=0.8,en;q=0.7','-H','Cache-Control: no-cache',url];
    execFile(process.platform==='win32'?'curl.exe':'curl',args,{encoding:null,maxBuffer:20*1024*1024},(err,stdout,stderr)=>{
      if(err) return reject(new Error((stderr&&String(stderr).trim())||err.message||'curl 失敗'));
      const buf=Buffer.isBuffer(stdout)?stdout:Buffer.from(stdout||'');
      const utf8=buf.toString('utf8');
      let text=utf8;
      if((utf8.match(/�/g)||[]).length>3) text=iconv.decode(buf,'gb18030');
      resolve(text);
    });
  });
}
async function fetchText(url){
  ...
  let last='未知錯誤';
  for(let i=1;i<=3;i++){
    try{const t=await fetchTextOnce(url); dpxqLastFetch=Date.now(); return t;}
    catch(e){last=e.message||String(e); if(i<3) await sleep(1000*i);}
  }
  throw new Error(`抓取逾時／連線失敗（已重試3次）：${last}`);
}
...
function decodeDpxqPosition(binit){
  ...
  const s=String(binit||'').replace(/[^0-9]/g,''); if(s.length<4)return pieces;
...
function parseDpxqIndex(html,baseUrl){
  ...
  while((m=re.exec(html))){const href=decodeHtml(m[1]),text=stripHtml(m[2]);if(!href||!text)continue;let detailUrl='';try{detailUrl=new URL(href,baseUrl).href;}catch{continue;}if(!/dpxq\.com\/hldcg\/search\/view_/i.test(detailUrl))continue;if(seen.has(detailUrl))continue;seen.add(detailUrl);const mm=text.match(/(.+?)\\s+(胜|負|负|和|和棋)\\s+(.+)/);let red='',black='',result='未知';if(mm){red=mm[1].trim();result=mm[2];black=mm[3].trim();}const ym=text.match(/\b(19|20)\d{2}\b/);out.push({title:text,red,black,result,year:ym?Number(ym[0]):null,detailUrl,source:'dpxq-index',sourceUrl:baseUrl,opening:'',moves:[],tokens:null,exactMoves:null,notes:'由東萍公開索引同步。'});if(out.length>=100)break;}
  return out;
}
...
async function fetchDetail(detailUrl){const html=await fetchText(detailUrl);const blocks=parseDpxqBlocks(html);const b=blocks.find(x=>x.movelist||x.red||x.black)||blocks[0];if(!b){const wxf=extractWxfTokens(html);return {html,b:{},dec:{tokens:wxf,exactMoves:[],valid:!!wxf.length,badAt:-1,reason:wxf.length?'WXF文字棋譜':'未找到棋譜資料',totalEncoded:wxf.length},format:wxf.length?'WXF':'unknown'};}const dec=b.movelist?decodeDpxqMoves(b):{tokens:extractWxfTokens(html),exactMoves:[],valid:false,badAt:-1,reason:'沒有 DhtmlXQ movelist，使用 WXF 文字解析',totalEncoded:0};return {html,b,dec,format:b.movelist?'DhtmlXQ':(dec.tokens.length?'WXF':'unknown')};}
...
async function runSync(job){
  try{
    ...
    while(queue.length&&page<job.maxPages&&!job.cancelled){
      ...
      const html=await fetchText(url);const items=parseDpxqIndex(html,url);let filtered=job.player?items.filter(x=>contains(x.red,job.player)||contains(x.black,job.player)||contains(x.title,job.player)):items;
      ...
      if(queue.length&&!job.cancelled)await sleep(1500);
    }
...
app.get('/api/dpxq/test',async(req,res)=>{const url=String(req.query.url||'https://www.dpxq.com/').trim();if(!isDpxqUrl(url))return res.status(400).json({ok:false,error:'只允許 dpxq.com 網址'});const t0=Date.now();try{const html=await fetchText(url);const games=parseDpxqIndex(html,url);const blocks=parseDpxqBlocks(html);return res.json({ok:true,url,ms:Date.now()-t0,bytes:Buffer.byteLength(html,'utf8'),indexGames:games.length,dhtmlxqBlocks:blocks.length,message:games.length?'已解析到棋譜索引':'頁面連線正常，但這一頁是分類／目錄頁，將由自動同步器繼續尋找棋譜列表。'});}catch(e){return res.status(502).json({ok:false,url,error:e.message});}});
app.get('/api/dpxq/source',(req,res)=>res.json({name:'東萍象棋網',url:'https://www.dpxq.com/',mode:'public-index-detail-sync',message:'同步公開索引與公開棋譜詳細頁；保留來源 URL。'}));
